...
#endif
		pexit(__FILE__, __LINE__,"memalign (\"Zu\" bytes)", size);
#elif HAVE___MINGW_ALIGNED_MALLOC
	if (!(ptr = __mingw_aligned_malloc(size, align)))
		pexit(__FILE__, __LINE__,"__mingw_aligned_malloc (%u bytes)", (unsigned)size);
#elif HAVE__ALIGNED_MALLOC
	if (!(ptr = _aligned_malloc(size, align)))
		pexit(__FILE__, __LINE__,"_aligned_malloc (\"Zu\" bytes)", size);
